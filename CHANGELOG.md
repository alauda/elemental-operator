...
#### Unreleased - Alauda branch

- Apply control-plane/master/cpaas-system NoSchedule tolerations to SeedImage builder Pods so ISO builds can schedule on control-plane-only clusters.
- Require an explicit Elemental `--server-url` and use it for machine registration URLs, SeedImage download URLs, and the system-agent Kubernetes URL.
- Add Elemental HTTP server and registration TLS options: `--http-bind-addr`, `--ca-cert-file`, `--agent-tls-mode`, and `--system-agent-cluster-name`.
- Propagate `--seedimage-image-pull-secrets` to SeedImage builder Pods after trimming empty entries and removing duplicate secret names.
- Mark `MachineInventory` network config ready immediately when no network reconciliation is needed (`spec.network.configurator` is empty or `none`).
- Add an opt-in system-agent authorization split between DR-synchronized `shared` and cluster-local `global` scopes with exact per-scope plan Secret RBAC, while preserving the legacy single shared identity by default.
- Support `baremetal.cluster.io/system-agent-direct` as a legacy alias for the direct API-server endpoint mode; the explicit endpoint-mode annotation takes precedence.
